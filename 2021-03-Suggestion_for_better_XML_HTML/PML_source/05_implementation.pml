[ch Implementation
    ...
    Having successfully used the PML syntax for some time, I was somewhat confident that the pXML syntax should work well for XML documents too. However, to eliminate any doubts, I wanted a [i proof of concept] for pXML, before publishing this article. Therefore I created a parser that reads the pXML syntax presented in this article. The parser is written in Java and has no dependencies. I will open-source it.
    
    The following features are currently implemented:

    [list
        [el Convert pXML into XML (meta-elements in pXML are converted into XML attributes, and pXML/XML escape rules are applied)]
        [el Convert XML into pXML (attributes in XML are converted into pXML meta-elements, and pXML/XML escape rules are applied)]
        [el
            ...
            [list
                ...
                [el insert, modify, and delete elements and attributes, and save the result as a new XML document]
    ...
    Once the pXML parser is ready to be open-sourced, I'll publish a dedicated article with more examples.

    If there is enough interest I might also create a dedicated pXML website with a syntax specification and the grammar expressed in EBNF. Everybody is very welcome to participate in an open-source project.
]
